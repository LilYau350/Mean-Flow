...
class FlowMatching:
    def __init__(
        ...
        args,
        path_type,         
        flow_ratio=0.75, #control how many time steps satisfy r=t, when flow ratio = 1.0,  mean flow degrades as flow matching
    ...
    def mean_flow_sample(self, model, z, device, num_steps=1, guidance_scale=1.0, **model_kwargs):
        start, end = self.interval
        t_vals = torch.linspace(start, end, num_steps + 1, device=device)
        ...
        for i in range(num_steps):
            ...
            # Get model's output with CFG applied
            u_combined = self.forward_with_cfg(model, z, t, guidance_scale, **model_kwargs)
            
            # Update z based on the formula z_r = z_t - (t - r) * u(z_t, r, t)
            z = z - (t_ - r_) * u_combined
    ...
    def sample(self, model, noise, device, num_steps=50, solver='heun', guidance_scale=1.0, **model_kwargs):
        if self.use_mean_flow:
            return self.mean_flow_sample(model, noise, device, num_steps, guidance_scale, **model_kwargs)
        else:
